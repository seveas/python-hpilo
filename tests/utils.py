    ...
    def __new__(cls, name, bases, attrs):
        ...
        for section in config.sections():
            ...
            if not ilo.protocol:
                ilo.protocol = (
                    hpilo.ILO_RAW
                    if ilo.firmware_version["management_processor"].lower()
                    in ("ilo", "ilo2")
                    else hpilo.ILO_HTTP
                )
            ...
            for method in methods:
                ...
                attrs[fname] = eval(
                    "lambda self: self._test_%s(self.ilos['%s'])" % (method, key)
                )
...
class IloTestCase(unittest.TestCase):
    ...
    def require_ilo(self, ilo, *ilos):
        ...
        raise unittest.SkipTest(
            "This test requires %s, not %s:%s"
            % (
                "|".join(ilos),
                ilo.firmware_version["management_processor"].lower(),
                ilo.firmware_version["firmware_version"],
            )
        )
