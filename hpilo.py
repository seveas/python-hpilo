...
# Python 2.6, and non-stdlib ElementTree
    ...
    raise RuntimeError(
        "Don't know how to monkeypatch XML serializer workarounds. Please report a bug at https://github.com/seveas/python-hpilo"
    )
...
if PY3:
    ...
    # the manual way.
    IloError = IloErrorMeta(
        "IloError",
        (Exception,),
        {"known_subclasses": [], "__init__": IloError.__init__},
    )
...
class Ilo(object):
    ...
    def __init__(
        ...
        self._protect_passwords = (
            os.environ.get("HPILO_DONT_PROTECT_PASSWORDS", None) != "YesPlease"
        )
        ...
        if self.ssl_verify:
            if sys.version_info < (2, 7, 9):
                raise EnvironmentError(
                    "SSL verification only works with python 2.7.9 or newer"
                )
    ...
    def _detect_protocol(self):
        ...
        header, data = self._communicate(
            b'<RIBCL VERSION="2.0"></RIBCL>', ILO_HTTP, save=False
        )
        if header:
            ...
            if len(part) < self.BLOCK_SIZE:
                ...
                while sent < fwlen:
                    ...
                    if written is None:
                        ...
                        raise IloCommunicationError(
                            "Unexpected EOF while sending %d bytes (%d of %d sent before)"
                            % (plen, sent, fwlen)
                        )
        ...
        try:
            ...
            if not data:
                raise IloCommunicationError(
                    ...
                    % (self.hostname, self.port, str(exc))
                )
    ...
    def _get_socket(self):
        ...
        if self.protocol == ILO_LOCAL:
            ...
            try:
                ...
                raise IloCommunicationError(
                    "Cannot run %s: %s" % (self.hponcfg, str(exc))
                )
        ...
        for res in socket.getaddrinfo(self.hostname, self.port, 0, socket.SOCK_STREAM):
            ...
            try:
                ...
                if sock is not None:
                    sock.close()
                err = IloCommunicationError(
                    "Timeout connecting to %s port %d" % (self.hostname, self.port)
                )
    ...
    def _communicate(self, xml, protocol, progress=None, save=True):
        ...
        if b"$EMBED" in xml:
            pre, name, post = (
                re.compile(b"(.*)\$EMBED:(.*)\$(.*)", re.DOTALL).match(xml).groups()
            )
            sock.write(pre)
            sent = 0
    ...
    def _root_element(self, element, **attrs):
        ...
        if not self.delayed or not self._elements:
            ...
            login = etree.SubElement(
                ...
                PASSWORD=DoNotEscapeMe(self.password),
            )
        if self.delayed:
            if self._elements:
        ...
        if (
            self.delayed
            and len(login)
            and login[-1].tag == element
            and login[-1].attrib == attrs
        ):
            element = login[-1]
        else:
    ...
    def _attempt_to_fix_broken_xml(self, data):
        ...
        warnings.warn(
            "iLO returned malformed XML, attempting to fix. Please contact HP to report a bug",
            IloXMLWarning,
        )
                ...
                # These are interesting, something went wrong
                    ...
                    if "syntax error" in child.get("MESSAGE") and not self.protocol:
                        ...
                        for subclass in IloError.known_subclasses:
                            if (
                                status in subclass.possible_codes
                                or message in subclass.possible_messages
                            ):
    ...
    def _element_children_to_dict(self, element):
        ...
        for elt in element:
            ...
            if hasattr(self, fname):
                ...
                continue
            key, val, unit, description = (
                elt.tag.lower(),
                elt.get("VALUE", elt.get("value", None)),
                elt.get("UNIT", None),
                elt.get("DESCRIPTION", None),
            )
            if val is None:
                ...
                # well, deal with it :)
                if (
                    element.tag.lower() == "rimp"
                    or elt.tag.lower() in self.xmldata_ectd.get(element.tag.lower(), [])
                    or elt.tag.lower() == "temps"
                ):
    ...
    def _element_to_list(self, element):
        ...
        if len(set(tagnames)) == 1:
            ...
            return [
                (child.tag.lower(), self._element_to_dict(child)) for child in element
            ]
    ...
    def _coerce(self, val):
        ...
        if isinstance(val, basestring):
            ...
            if val.isdigit():
                ...
                val = {"Y": True, "N": False, "true": True, "false": False}.get(
                    val, val
                )
        return val
    ...
    def _info_tag(
        self, infotype, tagname, returntags=None, attrib={}, process=lambda x: x
    ):
        root, inner = self._root_element(infotype, MODE="read")
        etree.SubElement(inner, tagname, **attrib)
        if self.delayed:
            self._processors.append(
                [self._process_info_tag, returntags or [tagname], process]
            )
            return
        header, message = self._request(root)
    ...
    def _control_tag(
        self, controltype, tagname, returntag=None, attrib={}, elements=[], text=None
    ):
        ...
        if self.delayed:
            if tagname == "CERTIFICATE_SIGNING_REQUEST":
                self._processors.append(
                    [self._process_control_tag, returntag or tagname]
                )
            return
        header, message = self._request(root)
    ...
    def add_sso_server(self, server=None, import_from=None, certificate=None):
        ...
        if [server, import_from, certificate].count(None) != 2:
            raise ValueError(
                "You must specify exactly one of server, import_from or certificate"
            )
        if server:
            return self._control_tag("SSO_INFO", "SSO_SERVER", attrib={"NAME": server})
        if import_from:
            return self._control_tag(
                "SSO_INFO", "SSO_SERVER", attrib={"IMPORT_FROM": import_from}
            )
        if certificate:
    ...
    def add_user(
        ...
        return self._control_tag(
            ...
            attrib={
                ...
                "USER_NAME": user_name,
                "PASSWORD": DoNotEscapeMe(password),
            },
        )
    ...
    def certificate_signing_request(
        ...
        return self._control_tag(
            "RIB_INFO", "CERTIFICATE_SIGNING_REQUEST", elements=elements
        )
    ...
    def delete_federation_group(self, group_name):
        ...
        return self._control_tag(
            "RIB_INFO", "DELETE_FEDERATION_GROUP", attrib={"GROUP_NAME": group_name}
        )

    def delete_sso_server(self, index):
        """Delete an SSO server by index"""
        return self._control_tag(
            "SSO_INFO", "DELETE_SERVER", attrib={"INDEX": str(index)}
        )

    def delete_user(self, user_login):
        """Delete the specified user from the ilo"""
        return self._control_tag(
            "USER_INFO", "DELETE_USER", attrib={"USER_LOGIN": user_login}
        )
    ...
    def eject_virtual_media(self, device="cdrom"):
        ...
        return self._control_tag(
            "RIB_INFO", "EJECT_VIRTUAL_MEDIA", attrib={"DEVICE": device.upper()}
        )
    ...
    def get_current_boot_mode(self):
        ...
        return self._info_tag(
            ...
            process=lambda data: data["boot_mode"],
        )
    ...
    def _parse_get_embedded_health_data_nic_information(self, element):
        return {
            "nic_information": [self._element_children_to_dict(elt) for elt in element]
        }
    ...
    def _parse_get_embedded_health_data_storage(self, element):
        ...
        for ctrl in element:
            if ctrl.tag == "DISCOVERY_STATUS":
                ret["%s_%s" % (key, ctrl.tag.lower())] = self._element_children_to_dict(
                    ctrl
                )["status"]
                continue
            data = {}
    ...
    def get_federation_all_groups_info(self):
            ...
            data = [
                dict(
                    [
                        (
                            key,
                            {"yes": True, "no": False}.get(
                                val["value"].lower(), val["value"]
                            ),
                        )
        ...
        return self._info_tag(
            "RIB_INFO", "GET_FEDERATION_ALL_GROUPS_INFO", process=process
        )
    ...
    def get_host_power_saver_status(self):
        ...
        return self._info_tag(
            "SERVER_INFO", "GET_HOST_POWER_SAVER_STATUS", "GET_HOST_POWER_SAVER"
        )
    ...
    def get_one_time_boot(self):
        ...
        return self._info_tag(
            "SERVER_INFO",
            "GET_ONE_TIME_BOOT",
            ("ONE_TIME_BOOT", "GET_ONE_TIME_BOOT"),
            process=process,
        )
    ...
    def get_pers_mouse_keyboard_enabled(self):
        ...
        return self._info_tag(
            ...
            process=lambda data: data["persmouse_enabled"],
        )

    def get_power_cap(self):
        """Get the power cap setting"""
        return self._info_tag(
            "SERVER_INFO", "GET_POWER_CAP", process=lambda data: data["power_cap"]
        )
    ...
    def get_product_name(self):
        ...
        return self._info_tag(
            "SERVER_INFO", "GET_PRODUCT_NAME", process=lambda data: data["product_name"]
        )
    ...
    def get_server_auto_pwr(self):
        ...
        return self._info_tag(
            "SERVER_INFO",
            "GET_SERVER_AUTO_PWR",
            process=lambda data: data["server_auto_pwr"],
        )
    ...
    def get_server_event_log(self):
        ...
        return self._info_tag(
            "SERVER_INFO", "GET_EVENT_LOG", "EVENT_LOG", process=process
        )
    ...
    def get_server_fqdn(self):
        ...
        return self._info_tag(
            ...
            "GET_SERVER_FQDN",
            "SERVER_FQDN",
            process=lambda fqdn: fqdn["value"],
        )
    ...
    def get_server_name(self):
        ...
        return self._info_tag(
            ...
            "GET_SERVER_NAME",
            "SERVER_NAME",
            process=lambda name: name["value"],
        )
    ...
    def get_server_power_on_time(self):
        ...
        return self._info_tag(
            ...
            "SERVER_POWER_ON_MINUTES",
            process=lambda data: int(data["value"]),
        )
    ...
    def get_supported_boot_mode(self):
        return self._info_tag(
            ...
            process=lambda data: data["supported_boot_mode"],
        )
    ...
    def get_uid_status(self):
        ...
        return self._info_tag(
            "SERVER_INFO", "GET_UID_STATUS", process=lambda data: data["uid"]
        )

    def get_user(self, user_login):
        """Get user info about a specific user"""
        return self._info_tag(
            "USER_INFO", "GET_USER", attrib={"USER_LOGIN": user_login}
        )
    ...
    def hotkey_config(
        ...
        del vars["self"]
        elements = [
            etree.Element(x.upper(), VALUE=vars[x]) for x in vars if vars[x] is not None
        ]
    ...
    def delete_ssh_key(self, user_login):
        ...
        return self._control_tag(
            "USER_INFO",
            "MOD_USER",
            attrib={"USER_LOGIN": user_login},
            elements=[etree.Element("DEL_USERS_SSH_KEY")],
        )
    ...
    def insert_virtual_media(self, device, image_url):
        ...
        return self._control_tag(
            ...
            attrib={"DEVICE": device.upper(), "IMAGE_URL": image_url},
        )

    def mod_encrypt_settings(
        self,
        user_login,
        password,
        ilo_group_name,
        ...
        primary_server_port,
        secondary_server_address=None,
        secondary_server_port=None,
    ):
    ...
    def mod_federation_group(
        self,
        group_name,
        new_group_name=None,
        group_key=None,
        admin_priv=None,
        remote_cons_priv=None,
        reset_server_priv=None,
        virtual_media_priv=None,
        config_ilo_priv=None,
        login_priv=None,
    ):
    ...
    def mod_global_settings(
        self,
        ilo_funct_enabled=None,
        rbsu_post_ip=None,
        # Access settings
        f8_prompt_enabled=None,
        f8_login_required=None,
        lock_configuration=None,
        serial_cli_status=None,
        serial_cli_speed=None,
        http_port=None,
        https_port=None,
        ssh_port=None,
        ssh_status=None,
        ipmi_dcmi_over_lan_enabled=None,
        ipmi_dcmi_over_lan_port=None,
        remote_console_port_status=None,
        remote_console_port=None,
        remote_console_encryption=None,
        rawvsp_port=None,
        vsp_software_flow_control=None,
        terminal_services_port=None,
        shared_console_enable=None,
        shared_console_port=None,
        remote_console_acquire=None,
        telnet_enable=None,
        ssl_empty_records_enable=None,
        remote_console_status=None,
        ribcl_status=None,
        virtual_media_status=None,
        webgui_status=None,
        webserver_status=None,
        # Security settings
        min_password=None,
        enforce_aes=None,
        authentication_failure_logging=None,
        authentication_failure_delay_secs=None,
        authentication_failures_before_delay=None,
        ssl_v3_enable=None,
        session_timeout=None,
        # Monitoring & alerting
        snmp_access_enabled=None,
        snmp_port=None,
        snmp_trap_port=None,
        remote_syslog_enable=None,
        remote_syslog_server_address=None,
        remote_syslog_port=None,
        alertmail_enable=None,
        alertmail_email_address=None,
        alertmail_sender_domain=None,
        alertmail_smtp_server=None,
        alertmail_smtp_port=None,
        alertmail_smtp_auth_enable=None,
        alertmail_smtp_auth_username=None,
        alertmail_smtp_secure_enable=None,
        # Console capturing
        vsp_log_enable=None,
        interactive_console_replay_enable=None,
        console_capture_enable=None,
        console_capture_boot_buffer_enable=None,
        console_capture_fault_buffer_enable=None,
        console_capture_port=None,
        capture_auto_export_enable=None,
        capture_auto_export_location=None,
        capture_auto_export_username=None,
        capture_auto_export_password=None,
        # And the rest
        remote_keyboard_model=None,
        virtual_kbmouse_connection=None,
        vmedia_disable=None,
        virtual_media_port=None,
        key_up_key_down_enable=None,
        high_performance_mouse=None,
        brownout_recovery=None,
        enhanced_cli_prompt_enable=None,
        tcp_keep_alive_enable=None,
        ...
        elements = [
            etree.Element(
                x.upper(), VALUE=str({True: "Yes", False: "No"}.get(vars[x], vars[x]))
            )
            for x in vars
            if vars[x] is not None and x not in dont_map
        ] + [
            etree.Element(x.upper(), VALUE=str(vars[x]))
            for x in vars
            if vars[x] is not None and x in dont_map
        ]
        return self._control_tag("RIB_INFO", "MOD_GLOBAL_SETTINGS", elements=elements)

    def mod_network_settings(
        self,
        enable_nic=None,
        reg_ddns_server=None,
        ping_gateway=None,
        dhcp_domain_name=None,
        speed_autoselect=None,
        nic_speed=None,
        full_duplex=None,
        dhcp_enable=None,
        ip_address=None,
        ...
        dhcp_wins_server=None,
        dhcp_static_route=None,
        reg_wins_server=None,
        prim_dns_server=None,
        sec_dns_server=None,
        ter_dns_server=None,
        prim_wins_server=None,
        sec_wins_server=None,
        static_route_1=None,
        static_route_2=None,
        static_route_3=None,
        dhcp_sntp_settings=None,
        sntp_server1=None,
        sntp_server2=None,
        timezone=None,
        enclosure_ip_enable=None,
        web_agent_ip_address=None,
        shared_network_port=None,
        vlan_enabled=None,
        vlan_id=None,
        shared_network_port_vlan=None,
        shared_network_port_vlan_id=None,
        ipv6_address=None,
        ipv6_static_route_1=None,
        ipv6_static_route_2=None,
        ipv6_static_route_3=None,
        ipv6_prim_dns_server=None,
        ipv6_sec_dns_server=None,
        ipv6_ter_dns_server=None,
        ipv6_default_gateway=None,
        ipv6_preferred_protocol=None,
        ipv6_addr_autocfg=None,
        ipv6_reg_ddns_server=None,
        dhcpv6_dns_server=None,
        dhcpv6_rapid_commit=None,
        dhcpv6_stateful_enable=None,
        dhcpv6_stateless_enable=None,
        dhcpv6_sntp_settings=None,
        dhcpv6_domain_name=None,
        ilo_nic_auto_select=None,
        ilo_nic_auto_snp_scan=None,
        ilo_nic_auto_delay=None,
        ilo_nic_fail_over=None,
        gratuitous_arp=None,
        ...
        elements = [
            ...
            for x in vars
            if vars[x] is not None and "static_route_" not in x and x not in dont_map
        ] + [
            etree.Element(x.upper(), VALUE=str(vars[x]))
            for x in vars
            if vars[x] is not None and "static_route_" not in x and x in dont_map
        ]
    ...
    mod_network_settings.requires_dict = [
        "static_route_1",
        "static_route_2",
        "static_route_3",
        "ipv6_static_route_1",
    ...
    def mod_dir_config(
        self,
        dir_authentication_enabled=None,
        dir_local_user_acct=None,
        dir_server_address=None,
        dir_server_port=None,
        dir_object_dn=None,
        dir_object_password=None,
        dir_user_context_1=None,
        dir_user_context_2=None,
        dir_user_context_3=None,
        dir_user_context_4=None,
        dir_user_context_5=None,
        dir_user_context_6=None,
        dir_user_context_7=None,
        dir_user_context_8=None,
        dir_user_context_9=None,
        dir_user_context_10=None,
        dir_user_context_11=None,
        dir_user_context_12=None,
        dir_user_context_13=None,
        dir_user_context_14=None,
        dir_user_context_15=None,
        dir_enable_grp_acct=None,
        dir_kerberos_enabled=None,
        dir_kerberos_realm=None,
        dir_kerberos_kdc_address=None,
        dir_kerberos_kdc_port=None,
        dir_kerberos_keytab=None,
        dir_generic_ldap_enabled=None,
        dir_grpacct1_name=None,
        dir_grpacct1_sid=None,
        dir_grpacct1_priv=None,
        dir_grpacct2_name=None,
        dir_grpacct2_sid=None,
        dir_grpacct2_priv=None,
        dir_grpacct3_name=None,
        dir_grpacct3_sid=None,
        dir_grpacct3_priv=None,
        dir_grpacct4_name=None,
        dir_grpacct4_sid=None,
        dir_grpacct4_priv=None,
        dir_grpacct5_name=None,
        dir_grpacct5_sid=None,
        dir_grpacct5_priv=None,
        dir_grpacct6_name=None,
        dir_grpacct6_sid=None,
        dir_grpacct6_priv=None,
    ):
    ...
    def mod_snmp_im_settings(
        self,
        snmp_access=None,
        web_agent_ip_address=None,
        snmp_address_1=None,
        snmp_address_1_rocommunity=None,
        snmp_address_1_trapcommunity=None,
        snmp_address_2=None,
        snmp_address_2_rocommunity=None,
        snmp_address_2_trapcommunity=None,
        snmp_address_3=None,
        snmp_address_3_rocommunity=None,
        snmp_address_3_trapcommunity=None,
        snmp_port=None,
        snmp_trap_port=None,
        snmp_v3_engine_id=None,
        snmp_passthrough_status=None,
        trap_source_identifier=None,
        os_traps=None,
        rib_traps=None,
        cold_start_trap_broadcast=None,
        snmp_v1_traps=None,
        cim_security_mask=None,
        snmp_sys_location=None,
        snmp_sys_contact=None,
        agentless_management_enable=None,
        snmp_system_role=None,
        snmp_system_role_detail=None,
        snmp_user_profile_1=None,
        snmp_user_profile_2=None,
        snmp_user_profile_3=None,
    ):
        ...
        elements = [
            ...
            and "trapcommunity" not in x
            and "snmp_user_profile" not in x
        ]
        for key in vars:
    ...
    mod_snmp_im_settings.requires_dict = [
        "snmp_user_profile_1",
        "snmp_user_profile_2",
        "snmp_user_profile_3",
    ...
    def mod_sso_settings(
        self,
        trust_mode=None,
        user_remote_cons_priv=None,
        user_reset_server_priv=None,
        user_virtual_media_priv=None,
        user_config_ilo_priv=None,
        user_admin_priv=None,
        operator_login_priv=None,
        operator_remote_cons_priv=None,
        operator_reset_server_priv=None,
        operator_virtual_media_priv=None,
        operator_config_ilo_priv=None,
        operator_admin_priv=None,
        administrator_login_priv=None,
        administrator_remote_cons_priv=None,
        administrator_reset_server_priv=None,
        administrator_virtual_media_priv=None,
        administrator_config_ilo_priv=None,
        administrator_admin_priv=None,
    ):
        vars = dict(locals())
        del vars["self"]
        del vars["trust_mode"]
        ...
        vars = [
            (x.upper().split("_", 1), {True: "Yes", False: "No"}.get(vars[x], vars[x]))
            for x in vars
            if vars[x]
        ]
        elements += [
            etree.Element(x[0][0] + "_ROLE", attrib={x[0][1]: x[1]}) for x in vars
        ]
        return self._control_tag("SSO_INFO", "MOD_SSO_SETTINGS", elements=elements)

    def mod_twofactor_settings(
        self,
        auth_twofactor_enable=None,
        cert_revocation_check=None,
        cert_owner_san=None,
        ...
        if auth_twofactor_enable is not None:
            elements.append(
                etree.Element(
                    "AUTH_TWOFACTOR_ENABLE",
                    VALUE=["No", "Yes"][bool(auth_twofactor_enable)],
                )
            )
        if cert_revocation_check is not None:
            elements.append(
                etree.Element(
                    "CERT_REVOCATION_CHECK",
                    VALUE=["No", "Yes"][bool(cert_revocation_check)],
                )
            )
        if cert_owner_san:
        ...
        return self._control_tag(
            "RIB_INFO", "MOD_TWOFACTOR_SETTINGS", elements=elements
        )
    ...
    def mod_user(
        ...
        if attrs["password"] is not None:
            elements.append(
                etree.Element("PASSWORD", VALUE=DoNotEscapeMe(attrs["password"]))
            )
        for attribute in [x for x in attrs.keys() if x.endswith("_priv")]:
            if attrs[attribute] is not None:
                val = ["No", "Yes"][bool(attrs[attribute])]
    ...
    def profile_apply(self, desc_name, action):
        ...
        elements = [
            ...
            etree.Element(
                "PROFILE_OPTIONS", attrib={"VALUE": "none"}
            ),  # Currently unused
    ...
    def profile_delete(self, desc_name):
        ...
        return self._control_tag(
            "RIB_INFO",
            "PROFILE_DELETE",
            elements=[etree.Element("PROFILE_DESC_NAME", attrib={"VALUE": desc_name})],
        )

    def profile_desc_download(
        self,
        desc_name,
        name,
        description,
        ...
        elements = [
            ...
            etree.Element(
                "PROFILE_SCHEMA", attrib={"VALUE": "intelligentprovisioning.1.0.0"}
            ),
        ]
        if blob_namespace:
            elements.append(
                etree.Element("BLOB_NAMESPACE", attrib={"VALUE": blob_namespace})
            )
        if blob_name:
    ...
    def profile_list(self):
        ...
        return self._info_tag(
            "RIB_INFO", "PROFILE_LIST", "PROFILE_DESC_LIST", process=process
        )
    ...
    def set_asset_tag(self, asset_tag):
        ...
        return self._control_tag(
            "SERVER_INFO", "SET_ASSET_TAG", attrib={"VALUE": asset_tag}
        )
    ...
    def set_critical_temp_remain_off(self, value):
        ...
        return self._control_tag(
            "SERVER_INFO", "SET_CRITICAL_TEMP_REMAIN_OFF", attrib={"VALUE": value}
        )

    def set_ers_direct_connect(
        self,
        user_id,
        password,
        proxy_url=None,
        ...
        for key, value in locals().items():
            if key.startswith("proxy_") and value is not None:
                elements.append(
                    etree.Element("ERS_WEB_" + key, attrib={"VALUE": str(value)})
                )
        return self._control_tag(
            "RIB_INFO", "SET_ERS_DIRECT_CONNECT", elements=elements
        )
    ...
    def set_ers_irs_connect(self, ers_destination_url, ers_destination_port):
        ...
        elements = [
            etree.Element(
                "ERS_DESTINATION_URL", attrib={"VALUE": str(ers_destination_url)}
            ),
            etree.Element(
                "ERS_DESTINATION_PORT", attrib={"VALUE": str(ers_destination_port)}
            ),
        ]
        return self._control_tag("RIB_INFO", "SET_ERS_IRS_CONNECT", elements=elements)

    def set_ers_web_proxy(
        self, proxy_url, proxy_port, proxy_username=None, proxy_password=None
    ):
        """Register your iLO with HP Insigt Online using Direct Connect. Note
        that you must also call dc_registration_complete"""
        elements = []
    ...
    def set_federation_multicast(
        self,
        multicast_federation_enabled=True,
        multicast_discovery_enabled=True,
        multicast_announcement_interval=600,
        ipv6_multicast_scope="Site",
        multicast_ttl=5,
    ):
        """Set the Federation multicast configuration"""
        multicast_federation_enabled = {True: "Yes", False: "No"}[
            multicast_federation_enabled
        ]
        multicast_discovery_enabled = {True: "Yes", False: "No"}[
            multicast_discovery_enabled
        ]
        elements = [
            etree.Element(
                "MULTICAST_FEDERATION_ENABLED",
                attrib={"VALUE": multicast_federation_enabled},
            ),
            etree.Element(
                "MULTICAST_DISCOVERY_ENABLED",
                attrib={"VALUE": multicast_discovery_enabled},
            ),
            etree.Element(
                "MULTICAST_ANNOUNCEMENT_INTERVAL",
                attrib={"VALUE": str(multicast_announcement_interval)},
            ),
            etree.Element(
                "IPV6_MULTICAST_SCOPE", attrib={"VALUE": str(ipv6_multicast_scope)}
            ),
            etree.Element("MULTICAST_TTL", attrib={"VALUE": str(multicast_ttl)}),
        ]
        return self._control_tag(
            "RIB_INFO", "SET_FEDERATION_MULTICAST", elements=elements
        )

    def set_language(self, lang_id):
        """Set the default language. Only EN, JA and ZH are supported"""
        return self._control_tag(
            "RIB_INFO", "SET_LANGUAGE", attrib={"LANG_ID": lang_id}
        )
    ...
    def set_host_power(self, host_power=True):
        ...
        return self._control_tag(
            "SERVER_INFO", "SET_HOST_POWER", attrib={"HOST_POWER": power}
        )
    ...
    def set_host_power_saver(self, host_power_saver):
        ...
        host_power_saver = str(
            mapping.get(str(host_power_saver).lower(), host_power_saver)
        )
        return self._control_tag(
            "SERVER_INFO",
            "SET_HOST_POWER_SAVER",
            attrib={"HOST_POWER_SAVER": host_power_saver},
        )
    ...
    def set_one_time_boot(self, device):
        ...
        if not device.lower().startswith("boot"):
            device = device.upper()
        return self._control_tag(
            "SERVER_INFO", "SET_ONE_TIME_BOOT", attrib={"VALUE": device}
        )

    def set_pending_boot_mode(self, boot_mode):
        """Set the boot mode for the next boot to UEFI or legacy"""
        return self._control_tag(
            "SERVER_INFO", "SET_PENDING_BOOT_MODE", attrib={"VALUE": boot_mode.upper()}
        )
    ...
    def set_persistent_boot(self, devices):
        ...
        return self._control_tag(
            "SERVER_INFO", "SET_PERSISTENT_BOOT", elements=elements
        )
    ...
    def set_pers_mouse_keyboard_enabled(self, enabled):
        ...
        return self._control_tag(
            "SERVER_INFO", "SET_PERS_MOUSE_KEYBOARD_ENABLED", attrib={"VALUE": enabled}
        )
    ...
    def set_power_cap(self, power_cap):
        ...
        return self._control_tag(
            "SERVER_INFO", "SET_POWER_CAP", attrib={"POWER_CAP": str(power_cap)}
        )
    ...
    def set_server_auto_pwr(self, setting):
        ...
        return self._control_tag(
            "SERVER_INFO", "SERVER_AUTO_PWR", attrib={"VALUE": setting}
        )
    ...
    def set_server_name(self, name):
        ...
        try:
            return self._control_tag(
                "SERVER_INFO", "SERVER_NAME", attrib={"VALUE": name}
            )
        except IloError:
    ...
    def set_vm_status(
        self, device="cdrom", boot_option="boot_once", write_protect=True
    ):
    ...
    def trigger_bb_data(self, message_id, days):
        ...
        return self._control_tag(
            "RIB_INFO",
            "TRIGGER_BB_DATA",
            attrib={"MESSAGE_ID": message_id, "BB_DAYS": days},
        )
    ...
    def trigger_l2_collection(self, message_id):
        ...
        return self._control_tag(
            "RIB_INFO", "TRIGGER_L2_COLLECTION", elements=[element]
        )
    ...
    def uid_control(self, uid=False):
        ...
        if isinstance(uid, basestring):
            uid = {"on": True, "yes": True, "off": False, "no": False}.get(
                uid.lower(), uid
            )
        uid = ["No", "Yes"][bool(uid)]
        return self._control_tag(
            "SERVER_INFO", "UID_CONTROL", attrib={"UID": uid.title()}
        )
        ...
        if not filename:
            ...
            if version == "latest":
                if ilo not in config:
                    raise IloError(
                        "Cannot update %s to the latest version automatically" % ilo
                    )
    ...
    def xmldata(self, item="all"):
        ...
        if item.lower() not in ("all", "cpqkey"):
            raise IloError(
                "unsupported xmldata argument '%s', must be 'all' or 'cpqkey'" % item
            )
        ...
        if self.read_response:
            ...
            if self.ssl_context:
                opener = urllib2.build_opener(
                    ...
                    urllib2.HTTPSHandler(context=self.ssl_context),
                )
                ...
                opener = urllib2.build_opener(
                    urllib2.ProxyHandler({}), urllib2.HTTPSHandler(context=ctx)
                )
            else:
    ...
    _parse_infra2_blades = lambda self, element: self._parse_infra2_XXXX(
        element, "blades", "blade"
    )
    _parse_infra2_switches = lambda self, element: self._parse_infra2_XXXX(
        element, "switches", "switch"
    )
    _parse_infra2_managers = lambda self, element: self._parse_infra2_XXXX(
        element, "managers", "manager"
    )
    _parse_infra2_lcds = lambda self, element: self._parse_infra2_XXXX(
        element, "lcds", "lcd"
    )
    _parse_infra2_fans = lambda self, element: self._parse_infra2_XXXX(
        element, "fans", "fan"
    )
